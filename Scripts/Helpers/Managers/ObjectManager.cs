...
public class ObjectManager : MonoBehaviour
{
    ...
    public ParticleSystem getParticle(ParticleType particleType)
    {
        ...
        {
            if (particlePool.particleType != particleType) continue;

            foreach (ParticleSystem particleSystem in particlePool.particlePool)
            {
                if (!particleSystem.isPlaying)
                    return particleSystem;
            }

            GameObject particleObj = Instantiate(particlePool.particle, particlePool.parent);
            particleObj.SetActive(false);
            Destroy(particleObj, destroyDelay);
